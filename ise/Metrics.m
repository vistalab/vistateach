...
% PURPOSE:
% 
%    The purpose of this tutorial is to explore some of the
%    properties of the CIELAB color metric.  In this tutorial, you will
% 
...
% We begin with an examination of some of the properties of the
% CIELAB color metric.  This metric is designed to help predict
% the discriminability between spatially uniform targets.

% Let's assume we have a set of gray surfaces whose reflectance
% functions are linearly scaled with respect to one another.
wavelength = 370:730;
graySurfaces = ones(361,10)*diag([.1:.1:1]);
plot(wavelength,graySurfaces)
set(gca,'xlim',[350 750]);
xlabel('wavelength (nm)'); ylabel('Reflectance')
title('Spectral Reflectance of 10 Gray Surfaces');

% Load a light source.  65 refers to the color
% temperature, namely 6500 deg Kelvin.  This is a slightly bluish
% appearing light source.

load cMatch/D65  % changed from 'load D65' (1/12/06)
lgt = D65; 
...
% Next, let's convert these values to LAB values.  We call the
% routine xyz2lab.  You should take a look at the routine to see
% what it does by invoking "type xyz2lab"

grayLAB = vcXYZ2lab( grayXYZ.', whiteXYZ.' ).'
...
% Again, we plot in 3 dimensions for LAB space.  Notice that the a* and b* 
% coordinates are zero for the gray series.
figure;
...
% Also, let's examine the chromaticity coordinates of these various
% surfaces.  Remember that the chromaticity coordinates represent only two
% of the three dimensions.  In this case, the surfaces all share a common
% "shape" spectral power distribution, and they differ only by a scale
% factor.  So, the surfaces will have the same chromaticity coordinates.
...
% To see what these Lab values look like, we will (a) convert the
% Lab values into XYZ values, (b) guess about your screen
% phosphors and gamma to render these XYZ values.  The techniques
...
varyXYZ = lab2xyz(varyAstarLAB',whiteXYZ)'
...
% To render these XYZ values we must load up some calibration
% information about your monitor.  Because I have no idea what
% monitor you are using -- this is a general problem for the
% industry -- I am going to just make a guess and use a standard
% set of monitor values.

load cMatch/monitor; comment
load cMatch/monitorGam

% Let's get the XYZ color matching functions.  We will need them.
...
% Then, compute the linear rgb values for the collection of
...
fbRGB = round(dac2rgb(varyRGB',invGamTable))

% We show the image as an indexed image.  We make a list of
% entries (1:N) for the nChip colors and we assign each entry the
% frame buffer values we have computed.  
figure(1);
mp = fbRGB/255; img = 1:nChips; image(img), colormap(mp)
...
varyXYZ = lab2xyz(varyBstar',whiteXYZ)'

% Convert the XYZ to linear rgb

varyRGB = xyz2rgb*varyXYZ
varyRGB = varyRGB/whiteMax;

% Convert the linear rgb to frame buffer values

fbRGB = round(dac2rgb(varyRGB',invGamTable))

% Make a picture
figure,
mp = fbRGB/255; img = 1:size(fbRGB,1); image(img), colormap(mp)
title('Image of B* Ramp (approximate)');
axis off


% By the way: do you see a visual illusion in these images?  The
% non-constant edges (the stimulus is constant) are called
% colored Mach Bands.
...
% 
lab1 = vcXYZ2lab( grayXYZ.', whiteXYZ.' ).';
lab2 = vcXYZ2lab( ( grayXYZ + deltaXYZ ).', whiteXYZ.' ).';
diffLAB = lab1 - lab2;
...
%
figure; plot3(lab1(2,:),lab1(3,:),lab1(1,:),'-o',...
...
% vcXYZ2lab may take an nx3 matrix of XYZ values to LAB values 
