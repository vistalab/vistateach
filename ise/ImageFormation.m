...
% Author:    Wandell
% Purpose:   Introduce visual angle, linespread, pointspread,
...
% visual angle, consider a printer whose dots are spaced (dots per inch) 
dpi = 600
...
% As you will see later in the course, experiments have shown
% that people can localize the position of a line to a spatial
% position of roughly 6 sec of visual angle.  Hence, at this
% viewing distance and with this many dots per inch, the dot
...
% dot every 30 sec. (i.e., roughly 28.58)
...
% residual variation in the retinal image.  Because of the
% blurring, the retinal image is much more likely the image of a
% bar than it is the image of a set of individual lines. 

% In fact, the dots placed on the page are not perfect line
% samples.  Each ink line has some width.  So, a more realistic
% input image might be created by blurring the stimulus and then
% convolving with the linespread.

gKernel = gausswin(30,2)';  % This produces a little Gaussian window for filtering
...
% And here is a comparison of the MTFs with the Williams data superimposed
% 
load williams
...
% 1. VIEWING GEOMETRY
%
% a) Write a matlab function "space2dpi" that computes dots per inch (dpi)
% from dot spacing (in sec of visual angle) and viewing distance (in
% inches).  Then write a function dpi2space that computes dot spacing (in
% sec of visual angle) from dpi and viewing distance (in inches).  Neither
% function should be more than ten lines.
%
% b) Experiments have shown that people can localize the position of a line
% to a spatial resolution of 6 sec of visual angle.  How many dots per inch
% are needed to obtain this localization resolution at a viewing distance
% of 12 inches?  How about a viewing distance of 36 inches?
% 
%
% 2. LINE SPREAD CALCULATIONS
% 
% In the linespread section of this tutorial, we compute the retinal image
% of a set of equally spaced lines.  Use the figures generated in the
% section "Convolution of image and linespread" for the following
% calculations.  
%
% The image will appear to be composed of individual lines, rather than a
% uniformly printed area, if the modulations in the middle of the retinal
% image are detectable.  We would like to be able to calculate how close
% the lines must be so that the image appears continuous.
%
% We define the "local image contrast" as the percentage: 
% 100*(max - min) / mean, where the max, min, and mean are all in 
% a local region of the image.
%
% a) Calculate the image contrast near the middle of the ink-blurred retinal image.
%
% b) What is the principal local spatial frequency near the center of this
% image?
%
% c) Look ahead in chapter 7 of Foundations of Vision (FOV), and find a graph
% that shows the human sensitivity to contrast as a function of spatial
% frequency. From interpreting that curve, determine whether you think the
% individual lines will be visible.  Make sure to explain your answer.
%
%
% 3. MODERN LINESPREADS, POINTSPREADS, AND MTFs
%
% Run the tutorial section that generates the plots of the ijspeert()
% function using the default parameters.  
%
% a) Use the ijspeert function to plot the linespread and MTF using a pupil
% diameter of 8.0mm, and use them to characterize the image formation
% quality of the eye.  Does one pupil diameter produce a better image than
% the other?  Explain.  Also explain physically why this makes sense.
%
%
% 4. CHROMATIC ABERRATION - SIMULATION
%
% a) At the end of this tutorial, we studied chromatic aberration in the
% frequency domain.  Give the peak absorption wavelength (in nm) of each
% cone class (please look that up in the FOV textbook, and assuming that we
% cannot detect spatial frequencies when the MTF is smaller than 0.2,
% estimate the maximum spatial frequency that can be detected by each cone
% type.
%
% b) Given the highest spatial frequency incident on the retina, how many L, M,
% and S cones are needed to sample the central 1 degree of the visual field
% without aliasing (Nyquist)? Does this number of cone samples make sense
% compared to the real number of L:M:S cones summarized in the text and
% lecture notes?
%
% c) Construct images of two sinusoids, one at 2 cycles per degree of
% visual angle (cpd) and the other at 8 cpd.  These images should vary
% along the x direction but be constant in the y-direction.  Arrange the
% sinusoidal intensity so that it falls between 0 and 1 (mean=0.5).
% Suppose that we sample each image only with S cone sampling rate (from
% your answer in part a).  What will the resulting sampled images look like
% if the eyes (i) have or (ii) do not have chromatic aberration?
%
